param(
  ...
  [string]$Branch = "main",
  [string]$CommitMessage = "Deploy TAS v70.20.3 Commercial Delivery Studio",
...
Write-Host "============================================================"
Write-Host "TAS v70.20.3 COMMERCIAL DELIVERY STUDIO ONE-CLICK RELEASE"
Write-Host "============================================================"
...
if ($LASTEXITCODE -ne 0) { Fail "npm ci failed in the v70.20.3 package." }
& npm.cmd run build
if ($LASTEXITCODE -ne 0) { Fail "The v70.20.3 package build or regression tests failed." }
...
if ($dirtyState.Count -gt 0) {
  $backupLabel = "TAS v70.20.3 automatic backup " + (Get-Date -Format "yyyy-MM-dd_HH-mm-ss")
...
if ($packageResolved -ieq $repoResolved) {
  Fail "The source package folder and Git repository folder must be separate. Keep the package at C:\TAS\TAS_v70_20_3_COMMERCIAL_DELIVERY_STUDIO and the repository at C:\TAS\tas."
}
...
$releaseFolders = @("src", "public", "functions", "scripts", "preview", "config", "editions", "release-assets", "demo-automation")
...
$repoRequiredFiles = @(
  ...
  "demo-automation/requirements.txt",
...
if ($hasNoChanges) {
  ...
  Write-Host "[7/10] Committing TAS v70.20.3 Commercial Delivery Studio..." -ForegroundColor Cyan
...
if (-not $GitOnly) {
  ...
  Write-Host "Release marker: https://$PagesHost/DEPLOYMENT_MARKER_TAS_V70_20_3_COMMERCIAL_DELIVERY.txt"
}
Write-Host ""
Write-Host "TAS v70.20.3 Commercial Delivery Studio has passed public-boundary, Demo, installer and independent-edition validation." -ForegroundColor Green
Wait-ForDan
